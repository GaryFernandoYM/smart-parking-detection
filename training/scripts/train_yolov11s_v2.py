"""
YOLOv11s — 10 runs con seeds diferentes
"""
import numpy as np
import torch
import json
...
SAVE_DIR  = f"{BASE}/modelos/v2/yolov11s"
SAVE_JSON = f"{BASE}/modelos/v2/yolov11s_full.json"
DEVICE    = "cuda:1"
...
for i, seed in enumerate(SEEDS):
    print(f"\n{'='*60}")
    print(f"YOLOv11s v2 — Run {i+1}/10 (seed={seed})")
    print(f"{'='*60}")

    model = YOLO("yolo11s.pt")
    model.train(
        data=DATA_YAML, epochs=100, imgsz=640, batch=16,
...
with open(SAVE_JSON, "w") as f:
    json.dump({
        "model": "YOLOv11s", "dataset": "Smart Parking UPeU v2",
