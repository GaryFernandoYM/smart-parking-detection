"""
YOLOv8s — 10 runs con seeds diferentes
Entrena, evalúa (global + por clase) y mide latencia
Guarda todo en un solo JSON profesional
"""
import numpy as np
import torch
import json
...
SAVE_DIR  = f"{BASE}/modelos/v2/yolov8s"
SAVE_JSON = f"{BASE}/modelos/v2/yolov8s_full.json"
DEVICE    = "cuda:0"
...
for i, seed in enumerate(SEEDS):
    print(f"\n{'='*60}")
    print(f"YOLOv8s v2 — Run {i+1}/10 (seed={seed})")
    print(f"{'='*60}")
    ...
    print(f"\n✅ Run {i+1}/10 completado:")
    print(f"   mAP50={map50:.4f} mAP50-95={map5095:.4f} "
          f"P={prec:.4f} R={rec:.4f} F1={f1:.4f}")
    print(f"   FPS={fps:.1f} lat_p50={lat_p50:.2f}ms "
          f"lat_p95={lat_p95:.2f}ms GPU={gpu_mb:.0f}MB")
    ...
    with open(SAVE_JSON, "w") as f:
        json.dump({"model": "YOLOv8s", "runs": runs}, f, indent=2)
...
summary = {}
print(f"\n{'='*60}")
print("SUMMARY YOLOv8s v2 — 10 runs")
print(f"{'='*60}")
for k in keys:
